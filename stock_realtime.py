...
# 3. Dynamic Injecting CSS Color Palettes (With Clean Structural Layout)
if theme_mode:
    bg_app = "#0E1117"
    bg_card = "#161B22"
    border_color = "#21262D"
...
st.markdown(f"""
    ...
    /* Streamlit Clean DataFrame Table Modifications */
...
# 5. 3-COLUMN PROFESSIONAL DASHBOARD ARCHITECTURE (แบ่งเป็น 3 ฝั่งเท่าๆ กันอย่างสะอาดตา)
col_left_scan, col_center_chart, col_right_matrix = st.columns([2.6, 5.0, 2.4])

# --- 🛰️ ฝั่งที่ 1: คอนโซลสแกนเนอร์ความเร็วสูง (Left Column) ---
with col_left_scan:
    st.markdown("<div class='section-title'>⚡ TOP 10 ALPHA SCREENER</div>", unsafe_allow_html=True)
    ...
    def fetch_single_scanner_data(t):
        try:
            ...
            if not h.empty:
                ...
                return {"สินทรัพย์": t, "WINRATE": sim_win, "ราคาล่าสุด": close_val, "โซนรับสะสมที่ดี": f"{p_s1*0.996:,.1f}-{close_val*0.999:,.1f}"}
        except: return None
    ...
    def scan_global_markets_multithreaded(tickers):
        ...
        if not df.empty:
            ...
            df["WINRATE"] = df["WINRATE"].map("{:.1f}%".format)
            df["ราคาล่าสุด"] = df["ราคาล่าสุด"].map("{:,.1f}".format)
        return df
    ...
    if execute and ticker_input:
        try:
            ...
            if hist_chart.empty:
                ...
                # ── Mini Dashboard Caps Layout (บีบขนาดให้แถวเดียวเรียบร้อยขึ้น) ──
                ...
                # ข้อมูลส่งไปใช้ฝั่งขวา
                st.session_state['quant_data'] = {
                    "high_val": high_val, "low_val": low_val, "current_price": current_price, "stock": stock,
                    "risk_profile": risk_profile, "display_currency": display_currency, "R1": (2 * ((high_val + low_val + current_price) / 3)) - low_val
                }
...
with col_right_matrix:
    ...
    if 'quant_data' in st.session_state and execute:
        ...
        # จัดวางพิกัดเป้าหมายแบบคลีนๆ เรียงตัวลงมา
